...
function CopyButton({
    ...
    const displayLabel = localIsCopied ? (copiedLabel ?? label ?? 'Copied!') : (label ?? '');

    return (
        <motion.button
            data-slot="copy-button"
            className={cn(buttonVariants({ variant, size }), label && 'flex flex-row p-4', className)}
            onClick={handleCopy}
            {...(props as any)}
        >
            {displayLabel && <span className="w-full font-mono text-xs">{displayLabel}</span>}
